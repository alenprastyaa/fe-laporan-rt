    ...
    <div class="mx-auto max-w-screen-2xl bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-7.5">
        ...
        <div
          ...
          <div class="mb-4">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Kepatuhan Warga</h3>
          ...
          <p class="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base mb-6">
            ...
            Tersisa <span class="font-bold text-red-500">{{ stats.jumlah_warga_menunggak }}</span> warga yang belum
            lunas.
          </p>
          ...
          <div class="grid grid-cols-2 gap-4 border-t border-gray-100 dark:border-gray-800 pt-4">
            <div class="text-center p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20">
              <p class="mb-1 text-blue-600 dark:text-blue-400 text-xs font-medium">Total Kas Wajib</p>
            ...
            <div class="text-center p-2 rounded-lg bg-green-50 dark:bg-green-900/20">
              <p class="mb-1 text-green-600 dark:text-green-400 text-xs font-medium">Total Sukarela</p>
              <p class="text-sm font-bold text-gray-800 dark:text-white/90">
        ...
        <div class="col-span-12 xl:col-span-5 space-y-6">
          <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 sm:p-6">
            ...
            <div class="mt-4 space-y-3">
              ...
              <div class="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                ...
                <span class="text-sm font-bold text-red-500">{{ stats.jumlah_warga_menunggak }} Orang</span>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 sm:p-6">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Daftar Menunggak</h3>
            ...
            <div class="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-1">
              <div v-if="wargaBelumBayar.length === 0" class="text-center py-8">
                <div class="text-4xl mb-2">🎉</div>
                <p class="text-sm text-gray-500">Luar biasa! Semua warga sudah lunas.</p>
              </div>

              <div v-for="(warga, index) in wargaBelumBayar" :key="index"
                class="group flex items-center justify-between p-3 border border-gray-100 dark:border-gray-700/50 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all duration-200">

                <div class="flex items-start gap-3 min-w-0 flex-1">
                  <div
                    class="h-10 w-10 rounded-full bg-red-50 dark:bg-red-900/20 text-red-600 flex items-center justify-center font-bold text-sm shrink-0 border border-red-100 dark:border-red-800/30">
                    {{ warga.username.charAt(0).toUpperCase() }}
                  </div>

                  <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2">
                      <p class="text-sm font-bold text-gray-800 dark:text-white truncate">{{ warga.username }}</p>
                    </div>

                    <div class="mt-1.5 flex flex-wrap gap-1.5">
                      <template v-if="warga.bulan_menunggak">
                        <span v-for="(bulan, idx) in warga.bulan_menunggak.split(',')" :key="idx"
                          class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-red-50 text-red-700 border border-red-100 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800/30">
                          {{ bulan.trim() }}
                        </span>
                      </template>
                      <span v-else class="text-xs text-gray-400 italic">Data bulan tidak tersedia</span>
                    </div>
                  </div>
                </div>

                <button v-if="role === 'admin'" @click="sendWhatsAppReminder(warga)"
                  class="ml-2 p-2 rounded-lg bg-green-50 text-green-600 hover:bg-[#25D366] hover:text-white hover:shadow-md transition-all duration-200 flex-shrink-0 border border-green-100 dark:border-transparent dark:bg-green-900/20 dark:text-green-400 dark:hover:bg-[#25D366] dark:hover:text-white"
            ...
            <table class="min-w-full">
              ...
              <tbody>
                <tr v-if="recentActivity.length === 0">
                  <td colspan="4" class="py-4 text-center text-sm text-gray-500">Belum ada aktivitas terbaru</td>
                </tr>
...
const wargaSudahBayar = computed(() => {
  return Math.max(0, stats.value.total_warga - stats.value.jumlah_warga_menunggak)
})
...
const complianceSeries = computed(() => {
  ...
  const paidCount = total - stats.value.jumlah_warga_menunggak
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await axios.get('https://alentest.my.id/laporan/api/dashboard/summary', config)
    ...
    try {
      const recentRes = await axios.get('https://alentest.my.id/laporan/api/dashboard/recent', config)

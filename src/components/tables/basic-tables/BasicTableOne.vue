            ...
            <tr v-else v-for="user in users" :key="user.id"
              ...
              <td class="px-5 py-4 sm:px-6">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <span class="block font-medium text-gray-800 dark:text-white/90 text-sm">{{ user.username }}</span>
                    <span class="block text-gray-500 text-xs">ID: #{{ user.id }}</span>
                  </div>
                </div>
              </td>
      ...
      <div class="border-t border-gray-200 dark:border-gray-700 px-5 py-4 flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <button @click="changePage(pagination.currentPage - 1)" :disabled="pagination.currentPage <= 1"
            class="px-3 py-1 text-sm border rounded hover:bg-gray-100 disabled:opacity-50">Prev</button>
          <button @click="changePage(pagination.currentPage + 1)"
            :disabled="pagination.currentPage >= pagination.totalPage"
            class="px-3 py-1 text-sm border rounded hover:bg-gray-100 disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
...
// State Pagination
const pagination = reactive({ currentPage: 1, limit: 5, totalData: 0, totalPage: 1 })
...
const fetchUsers = async () => {
  ...
  try {
    ...
    const res = await axios.get('http://localhost:6500/api/auth/users', {
...
const submitForm = async () => {
  ...
  try {
    if (isEditMode.value) {
      // Logic UPDATE
      await axios.put(`http://localhost:6500/api/auth/users/${editId.value}`, form, { headers })
      Swal.fire('Berhasil', 'Data user diperbarui', 'success')
    } else {
      // Logic CREATE
      await axios.post('http://localhost:6500/api/auth/users', form, { headers })
...
const deleteUser = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      const token = localStorage.getItem('token')
      await axios.delete(`http://localhost:6500/api/auth/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
